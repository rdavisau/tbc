...
#pragma warning restore CS0414
        ...
        {
            if (asm.PeBytes is { } bytes)
                WithCompilation(c => c.AddReferences(MetadataReference.CreateFromImage(bytes)));
            else
                Logger.LogWarning("Can't add reference {@AssemblyReference} that doesn't have assembly data", asm);
        }
